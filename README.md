...
My favorite theme for customizing terminal as well as GUI applications is [Nord]. You can see my Nord theme customizations in the `dir_colors` folder, the `spicetify_data` folder, and my `kakrc` file.
...
Make sure you have the `gh` command (the [Github CLI]), [Ruby] and [Bundler] installed, in particular by changing the "update_configs.rb" file.
